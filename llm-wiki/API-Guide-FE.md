...
- **참가자 경로는 공개 상태(공개예정·진행중·완료)에서만 동작한다**(BE-16). 초안·검수대기·보관으로 바뀌면 Q&A·설문이 **없는 이벤트와 같은 404**를 준다 — 존재 여부를 흘리지 않기 위해서다. 열어 둔 화면에서 갑자기 404가 나면 "행사가 종료됐거나 비공개로 바뀌었다"로 안내하면 된다.
