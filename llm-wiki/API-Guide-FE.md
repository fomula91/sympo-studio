...
- **참가자 경로는 발행 상태가 `공개`일 때만 동작한다**(BE-16, BE-23에서 상태 모델 재정의 → [[0009-event-status-axes]]). `초안`·`보관`으로 바뀌면 Q&A·설문이 **없는 이벤트와 같은 404**를 준다 — 존재 여부를 흘리지 않기 위해서다. 열어 둔 화면에서 갑자기 404가 나면 "행사가 종료됐거나 비공개로 바뀌었다"로 안내하면 된다.
...
- **404 규약**: 초안·보관 상태와 존재하지 않는 slug는 **완전히 동일한 404**를 돌려준다(존재 여부 비노출). FE는 하나의 "페이지를 찾을 수 없습니다" 화면이면 된다.
...
- **경계는 소유권이 아니라 공개 상태다**(참가자 경로의 규칙). 초안·보관은 **없는 slug와
