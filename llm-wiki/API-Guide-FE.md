...
> **구현 경계** — `id=1` 같은 상수나 에디터 목업 값을 Q&A 호출에 쓰지 않는다. 참가자 페이지는 초기 공개 조회에서 받은 이벤트 `id`를 `Microsite`·Q&A 컴포넌트까지 prop으로 전달해 GET·POST 모두에 사용한다. 로컬 시드의 id=1은 테스트 데이터일 뿐, 화면 계약이 아니다.
