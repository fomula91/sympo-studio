...
| rate limit | **IP당 60초 3건 / 하루 30건** | 429 + 사유 |

- **429는 반드시 화면에 사유를 노출**한다(FE-3 완료 기준). 재시도 버튼은 60초 뒤 활성화 권장.
- `engage.qa`가 꺼진 이벤트는 403. 폴링 GET은 계속 동작한다(모더레이션 경로는 BE-10 예정).
- 클라이언트에서도 글자 수를 미리 검증하면 왕복을 아끼지만, **서버 검증이 정본**이다.
- 알려진 한계: rate limit 키가 IP 단독이라 **같은 Wi-Fi(NAT)의 참가자들이 버킷을 공유**한다 — BE-9에서 브라우저 토큰을 섞는 방향으로 재설계 예정이며, 그때 FE가 토큰을 실어 보내야 한다. FE-3 착수 전에 BE와 키 설계를 맞출 것.
