...
자료(`documents`)는 `displayName`(필수, 160자)·`tag`(40자)·`sessionId`(그 이벤트의 세션이거나 `null`)·`status`(`pending`\|`ready`). **파일은 다루지 않는다** — `r2_key`·크기·페이지 수는 업로드 경로(BE-6)가 채우고, 응답에는 키 대신 `hasFile`이 온다. **새 자료는 `ready`로 만들 수 없다**(파일이 없는데 "준비됨"으로 그려지는 것을 막는다).
