...
FE가 호출하는 경로 **16개**를 아래 표에 먼저 둔다(라우트 파일은 18개지만 `GET /api/auth/google`과 `GET /api/auth/callback/google`은 브라우저가 직접 이동하는 경로라 `fetch` 대상이 아니어서 뺐다).

**누가 쓰는가**가 핵심 구분이다.

- **참가자 경로** — 무인증이고, 발행 상태가 `공개`일 때만 동작한다.
- **운영자 쓰기 경로** — 이벤트 소유권 검사를 지난다(BE-12·BE-13). **남의 것이면 403이 아니라 404**다 — 존재 자체를 흘리지 않는다. 소유자가 없는 데모 이벤트(`owner_id IS NULL`)는 게스트 체험을 위해 열려 있다.
- **운영자 읽기 경로와 프리셋 쓰기** — **아직 인가 검사가 없어 누구나 호출할 수 있다.** 목록·상세·`/ops`·`POST /api/presets`가 여기 해당하며 BE-24·BE-25에서 잠근다.
...
- **참가자 경로는 발행 상태가 `공개`일 때만 동작한다**(BE-16, BE-23에서 상태 모델 재정의 → [[0009-event-status-axes]]). `초안`·`검수대기`·`보관`으로 바뀌면 Q&A·설문이 **없는 이벤트와 같은 404**를 준다 — 존재 여부를 흘리지 않기 위해서다. 열어 둔 화면에서 갑자기 404가 나면 "행사가 종료됐거나 비공개로 바뀌었다"로 안내하면 된다.
- `engage_qa`가 꺼져도 **질문 목록 조회는 계속 된다** — 토글은 "새 질문을 받는가"이지 "이미 받은 질문을 감추는가"가 아니다. 개별 질문을 내리는 것은 운영자 모더레이션(`status='hidden'`)의 일이다.
...
```jsonc
{
  ...
  "host": "좌장 서정우",
  "capacity": 120,
  "status": "공개",             // 참가자 경로에는 이 값만 온다 (나머지는 404)
  "phase": "당일",              // 예정 | 당일 | 종료 — event_date에서 파생, 날짜 미정이면 null
...
> **구현 경계** — `id=1` 같은 상수나 에디터 목업 값을 Q&A 호출에 쓰지 않는다. 참가자 페이지는 초기 공개 조회에서 받은 이벤트 `id`를 `Microsite`·Q&A 컴포넌트까지 prop으로 전달해 GET·POST 모두에 사용한다. 로컬 시드의 id=1은 테스트 데이터일 뿐, 화면 계약이 아니다. **구조적으로 강제된다** — `Microsite`의 `eventId`는 옵셔널이고, `preview` prop이 true면(스튜디오 에디터·뷰어 미리보기) `QaPanel` 자체를 렌더하지 않는다. 실제 이벤트 id를 넘길 수 있는 유일한 정상 경로는 `/[slug]` 참가자 공개 페이지뿐이다(2026-09-02 Codex 리뷰 P1로 발견 — 전엔 미리보기가 진짜 D1 시드 행사에 쓰기를 일으켰다).

### 목록 — `GET` (BE-8 읽기 통제 적용)

폴링 비용을 줄이는 장치가 셋 있다. **셋 다 선택이고, 안 써도 기존 동작 그대로다.**

| 장치 | 쓰는 법 | 효과 |
|---|---|---|
| 엣지 캐시 | 자동 (`Cache-Control: public, max-age=3`) | 동시 접속자가 많을수록 크다 — 같은 URL의 3초 내 요청을 엣지가 대신 받는다 |
| `?since=<createdAt>` | 마지막으로 받은 질문의 `createdAt`을 그대로 넘긴다 | 그 이후만 온다(대개 빈 배열). 형식이 틀리면 400 |
| `ETag`/`If-None-Match` | 응답의 `ETag`를 다음 요청 헤더에 싣는다 | 바뀐 게 없으면 304 + 본문 0바이트 |

**`since`를 쓰면 응답이 증분이므로 화면은 기존 목록에 병합해야 한다**(통째로 교체하면 목록이 비워진다).

`ETag`는 바이트만 아끼고 서버 읽기는 줄이지 않는다 — 태그를 계산하려면 어차피 조회해야 한다. 읽기 티어를 실제로 아끼는 건 엣지 캐시와 `since`다.

**폴링은 탭이 백그라운드면 멈추는 것이 좋다**(`document.hidden`) — FE-3이 이미 이렇게 구현돼 있다(`visibilitychange`로 정지, 2026-09-02 [[log]]). 4초 폴링을 하루 종일 켜두면 참가자 한 명이 2만 건 넘게 요청한다.
...
- 폴링은 3~5초 간격으로 하되, 이전 요청이 끝나기 전에 다음 요청을 겹치게 하지 않는다. 현재 질문 API에는 재검증용 ETag 계약이 없으므로, 구현 시 오래된 브라우저 캐시가 그대로 표시되지 않도록 `fetch`의 캐시 정책을 명시한다(현 시점에는 `cache: 'no-store'`). BE-8에서 ETag/`since` 계약이 추가되면 그 계약으로 바꾼다.
...
- 201 응답은 등록된 질문 한 건이다. 성공 시 이 객체를 목록 state에 바로 반영(또는 즉시 재조회)해 다음 폴링까지 기다리지 않게 한다.
- 질문 열기 버튼으로 입력 패널을 표시할 때는 `textarea`로 초점을 이동한다. 전송·조회 오류는 `role="alert"` 또는 `aria-live`로 알려 키보드·스크린리더 사용자도 상태 변화를 알 수 있게 한다.

## 설문 — `/api/events/[id]/survey` (BE-4)

`[id]`는 공개 조회 응답의 `id`. 문항 구성(어떤 `questionKey`를 쓸지)은 **FE-4가 정의하는 계약**이다 — 서버는 키 형식만 검증하고 목록을 강제하지 않으므로, 설문 구조를 바꿔도 서버 변경이 필요 없다.

### 제출 — `POST`

한 요청에 문항 여러 개를 배열로 보낸다. 2단 폼(몰아 제출)과 "세션 끝날 때마다 1문항"(단건 제출) 모두 같은 형태다.

```jsonc
// 요청 — x-client-token 헤더 **필수**(Q&A와 달리 없으면 400)
{ "answers": [
  { "questionKey": "overall_satisfaction", "answer": 5 },          // 행사 전체 문항
  { "questionKey": "session_rating", "answer": 4, "sessionId": 1 } // 세션별 문항
] }
// 201 응답: { "saved": 2 }
```

- **`x-client-token`이 필수**인 이유: 같은 사람의 재제출을 "응답 수정"으로 접는 판정이 토큰에서 나온다. 토큰 생성 코드는 Q&A 섹션과 동일(같은 토큰을 공유하면 된다).
- **재제출은 덮어쓰기**다 — 같은 (문항, 세션)에 다시 제출하면 409가 아니라 최신 답으로 수정된다. "수정하기" UI를 그대로 만들 수 있다.
- `answer`는 문자열 또는 숫자(별점·NPS는 숫자로 보내면 된다. 저장은 문자열로 통일).

| 제약 | 값 | 초과 시 |
|---|---|---|
| `answers` | 1~20개, 같은 (문항, 세션) 중복 불가 | 400 + 사유 |
| `questionKey` | 영문·숫자·`_` `-` `.` 64자 이내 | 400 + 사유 |
| `answer` | 1~300자 (숫자는 문자열 변환 후) | 400 + 사유 |
| `sessionId` | 해당 이벤트의 세션 id, 선택 | 400 + 사유 |
| rate limit (브라우저) | 토큰당 **60초 60행 / 하루 200행**(행 = 문항 1개 저장·수정) | 429 + 사유 |
| rate limit (네트워크) | IP당 **60초 400행 / 하루 2,000행** | 429 + 사유 |

- 정상 사용(전 문항 제출 + 몇 번의 수정)은 한도에 닿지 않는다. 429 사유 문구는 Q&A와 같은 규약으로 그대로 노출하면 된다.
- `engage.survey`가 꺼진 이벤트는 403.

### 집계 — `GET /api/events/[id]/survey/summary`

운영자 리포트(FE-5)의 입력. 참가자 화면에는 쓰지 않는다.
...
- **`?auth=email_conflict`가 하나 더 있습니다**(BE-26) — 그 이메일로 만들어진 계정이 이미 있어 **잇지 않고 거절**한 경우입니다. 이것만 사유를 드러내도 되는 예외라, 화면에 "이 이메일로 만들어진 계정이 이미 있습니다. 기존 로그인 방법을 사용해 주세요." 정도의 안내를 띄워 주세요 — 문구가 없으면 이 상태에 빠진 사람은 원인을 모른 채 계속 실패합니다([[Decisions/0010-account-link-key]]). 지금은 복구 수단이 없습니다(BE-30)
...
운영자 에디터 전용. **목록 전체를 보내면 서버가 그 상태로 맞춘다**(PUT) — 추가·수정·삭제·순서 변경이 요청 하나로 끝난다.

| 메서드 | 경로 | 용도 |
|---|---|---|
| PUT | `/api/events/[id]/sessions` | 아젠다 목록 저장. 응답 `{ sessions: [...] }` |
| PUT | `/api/events/[id]/documents` | 자료 목록(메타) 저장. 응답 `{ documents: [...] }` |

읽기는 별도 엔드포인트가 없다 — `GET /api/events/[id]`가 `sessions`와 `documents`를 함께 돌려준다.

**FE가 지켜야 할 것 셋**

1. **배열의 위치가 곧 순서다.** `order` 필드를 보내도 무시된다. 드래그 결과를 배열 순서로 반영해 통째로 보내면 된다.
2. **기존 항목은 `id`를 그대로 실어 보낸다.** `id`가 있으면 수정, 없으면 신규, **보내지 않은 기존 항목은 삭제**다. 응답에 확정된 목록(신규 항목의 발급된 `id` 포함)이 오므로 그걸 다음 저장의 입력으로 쓴다.
3. **아젠다 저장과 자료 저장은 서로를 건드리지 않는다.** 두 화면이 따로 저장해도 상대의 편집이 지워지지 않는다.

```jsonc
// PUT /api/events/1/sessions
{ "sessions": [
  { "id": 6, "time": "19:10", "title": "폐회 및 설문", "speaker": "운영진", "kind": "CLOSING" },
  {           "time": "20:00", "title": "새로 추가한 세션", "speaker": "신규 연자", "kind": "CASE" }
] }
```
...
|---|---|
| `title` | 필수, 120자 이하 |
| `time` | `HH:MM`(24시간) 또는 생략 |
| `speaker` | 80자 이하 또는 생략 |
| `kind` | `OPENING`·`LECTURE`·`PANEL`·`QA`·`CASE`·`CLOSING` (생략 시 `LECTURE`) |
| 목록 길이 | 60개 이하 |

자료(`documents`)는 `displayName`(필수, 160자)·`tag`(40자)·`sessionId`(그 이벤트의 세션이거나 `null`). **파일은 다루지 않는다** — `r2_key`·크기·페이지 수는 업로드 경로(BE-6)가 채우고, 응답에는 키 대신 `hasFile`이 온다.

**`status`는 보내도 무시됩니다**(BE-27) — 서버가 **파일 유무에서 파생**합니다(`r2_key`가 있으면 `ready`, 없으면 `pending`). 예전에는 입력이었고 검사가 신규 생성에만 있어서, 파일 없는 자료를 메타 저장 한 번으로 `ready`로 올릴 수 있었습니다(그러면 참가자 화면에 **열 수 없는 자료가 "준비됨"으로** 뜹니다). 보낸 값이 아니라 **응답의 `status`를 그대로 쓰세요** — 업로드가 끝나면 자동으로 `ready`가 되고, 그 뒤 이름만 바꿔 저장해도 `ready`가 유지됩니다.

**세션을 지우면 그 세션의 설문 응답도 함께 지워진다.** 남겨서 `session_id`만 `NULL`이 되면 "세션 3에 대한 평가"가 "행사 전체 평가"로 둔갑해 집계를 오염시킨다. 반면 **자료와 질문은 살아남고 세션 연결만 끊긴다**(자료는 다시 붙일 수 있고, 참가자가 남긴 질문은 지울 근거가 없다).

상태코드: `400` 입력 오류·남의 이벤트 세션 참조 · `404` 없는 이벤트.
...
>
> BE-12(auth 4종)는 이미 구현돼 위 로그인 절로 옮겨졌다 — 이 절에는 아직 안 만들어진 것만 남긴다.
...
- **매일 00:00 KST에 프로덕션 데이터가 전부 리셋된다**(공개 데모 정책). 낮에 만든 이벤트·질문은 다음 날 없다. 시드 상태: 이벤트 id=1(`meridian-arte-seoul-260815`, 진행중), 세션 id **1~6 고정**, 예시 질문 2건, 예시 설문 응답 5건(`overall_satisfaction`·`session_rating` — FE-4 확정 전의 예시 키).
