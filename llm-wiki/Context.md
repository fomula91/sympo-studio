...
- 발행 상태 3종 `초안|공개|보관`(저장) / 행사 시점 3종(`event_date`에서 **파생**) → [[0009-event-status-axes]]. `진행중`은 상태가 아니라 시점이다. `검수대기`는 FE-24에서 뺐다 — 승인자 개념이 계정 모델에 없어 아무도 지키지 않는 상태였다([[Archive/Closed-Tasks]] FE-24).
