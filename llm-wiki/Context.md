...
- **무료로 오랫동안 유지**가 하드 제약이다. ~~결제 수단을 등록하지 않는 것이 안전장치다.~~ **2026-09-02 변경**: R2 활성화에 결제 수단이 필요해 등록했다. 안전장치가 "카드 미등록"에서 **"대시보드 사용량 알람 + 코드 상한"**으로 바뀌었다 — 파일당 20MB, 이벤트당 자료 60개, 고아 R2 객체를 자정 Cron이 정리(BE-6).
