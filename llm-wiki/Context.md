...
- **서버에서 끝난 것**: BE-1(D1 스키마 7개 테이블 → [[0005-d1-schema]]) · BE-3(Q&A 저장·조회 + 남용 통제) · BE-4(설문 응답 수집·집계, PR #12 머지) · BE-7(참가자 공개 조회 `GET /api/public/[slug]`) · BE-9(rate limit 2층 키 → [[0006-rate-limit-key]]) · BE-11(2차 리뷰 10건) · BE-14(아젠다·자료 쓰기 API). 전부 프로덕션 실측까지.
- **리뷰 대기 중**: FE-1·2·7·8·9(PR #9) — 확정 지적 12건 전부 로컬에서 수정·검증 완료(테스트·lint·build·헤드리스 실측), 아직 push 전.
- **참가자 공개 페이지가 생겼다**: `/[slug]`(신규, FE-3)가 BE-7 응답을 받아 `Microsite`를 렌더하고 Q&A가 실제 D1 이벤트에 연결됐다. `PUBLIC_EVENT_ID=1` 하드코딩(스튜디오 mock id와 D1 실제 id가 연결된 적이 없어서 생긴 문제, Codex 리뷰 [[Reviews/2026-09-01-codex-fe-qa]])은 BE-12·13·FE-15(인증) 없이도 풀렸다 — BE-7이 이미 무인증·배포 완료였기 때문. 나머지 화면(설문 등)은 아직 목업. 계약은 [[API-Guide-FE]].
- **다음 한 걸음**: PR #9 push·머지. BE 다음 추천은 **BE-12**(인증 기반) — 운영자 CRUD가 인가 검사도 rate limit도 없이 열려 있다. 읽기 비용이 급하면 BE-8.
- 지금 착수 가능한 FE: **FE-15의 게스트 로컬 워크스페이스**와 FE-4(설문 재설계 + 수료증, 수료증 부분은 서버 없이 완결).
