...
- **화면은 아직 대부분 목업으로 돈다.** 서버는 준비됐고 FE 연결(FE-3·FE-5)이 남았다. 계약은 [[API-Guide-FE]]. FE-3(Q&A 연결)은 착수 중(WIP)이나, 이벤트 id가 `PUBLIC_EVENT_ID=1`에 고정된 구조적 문제가 Codex 리뷰([[Reviews/2026-09-01-codex-fe-qa]])로 확인됨 — 스튜디오의 mock `EventItem.id`와 D1의 실제 이벤트 id가 아직 연결된 적이 없어서다(단순 prop 전달로 못 고침, 아래 참고).
- **결정 필요**: BE-12·BE-13(인증)·FE-15(게스트 워크스페이스+가져오기)가 스튜디오 이벤트와 D1을 잇는 유일한 계획인데, 아직 착수 전이라 FE-3의 "실제 이벤트에 연결"이 지금 당장은 뭘 기준으로 삼아야 할지 정해지지 않았다. 참가자용 공개 라우트(`/[slug]`, BE-7 API 소비)를 먼저 만들어 그 응답의 id를 쓰는 안과, FE-15 계정 연결을 기다리는 안 중 선택이 남음.
- **다음 한 걸음**: FE-3의 이벤트 id 연결 방식 결정 → PR #9 push·머지. BE 다음 추천은 **BE-12**(인증 기반) — 운영자 CRUD가 인가 검사도 rate limit도 없이 열려 있다. 읽기 비용이 급하면 BE-8.
- 지금 착수 가능한 FE: **FE-15의 게스트 로컬 워크스페이스**와 FE-4의 수료증 부분(둘 다 서버 없이 완결).
