-- 0010_auth — Google SSO 계정과 세션, 이벤트 소유권 (BE-12, ADR 0007)
--
-- 번호 주의: 이 파일은 원래 `0007_auth.sql`로 작성됐다가 브랜치가 오래 묵는 사이
-- 0008·0009가 먼저 적용돼, 되살리면서 0010으로 옮겼다. 앞 번호를 그대로 두면
-- "이미 적용된 0009 뒤에 0007이 적용되는" 읽기 어려운 이력이 남는다. 내용은
-- 0008·0009와 독립이라 순서 자체에는 의존이 없다.
--
