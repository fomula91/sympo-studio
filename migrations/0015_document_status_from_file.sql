-- 0015_document_status_from_file — 자료 status를 파일 존재와 일치시킨다 (BE-27)
--
-- **0011로 쓰였다가 0015로 옮겼다.** 이 마이그레이션이 미머지 브랜치에 묵는 사이
-- 0012~0014가 먼저 적용돼(원격 포함), 앞 번호를 두면 "적용된 0014 뒤에 0011이
-- 적용되는" 읽기 어려운 이력이 남는다. 내용이 앞 셋과 독립이라 순서 의존은 없다.
-- BE-12가 0007→0010으로 옮긴 것과 같은 이유다([[Context]] 마이그레이션 절).
--
